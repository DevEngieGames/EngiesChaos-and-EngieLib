...
public class IdeagiverEngieBundleInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	...
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == EngiesChaosModItems.CONTRIBUTOR_ENGIE_BUNDLE.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(100) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != EngiesChaosModItems.CONTRIBUTOR_ENGIE_BUNDLE.get();
			}
