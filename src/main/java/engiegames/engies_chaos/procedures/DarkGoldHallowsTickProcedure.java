...
public class DarkGoldHallowsTickProcedure {
	public static void execute(LevelAccessor world, Entity entity, ItemStack itemstack) {
		if (entity == null)
			return;
		if (itemstack.getOrCreateTag().getDouble("engiegameshallowscythestatclockcount") > 0) {
			itemstack.getOrCreateTag().putDouble("darkgoldhallowskillcount", (itemstack.getOrCreateTag().getDouble("engiegameshallowscythestatclockcount")));
			itemstack.getOrCreateTag().putDouble("engiegameshallowscythestatclockcount", 0);
		}
		if (itemstack.getOrCreateTag().getDouble("darkgoldhallowskillcount") < 10000) {
			itemstack.getOrCreateTag().putBoolean("givenupgradeitem", false);
		} else if (itemstack.getOrCreateTag().getDouble("darkgoldhallowskillcount") >= 10000) {
