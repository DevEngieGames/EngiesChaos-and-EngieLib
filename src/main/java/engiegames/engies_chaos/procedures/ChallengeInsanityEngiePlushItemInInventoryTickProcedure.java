...
public class ChallengeInsanityEngiePlushItemInInventoryTickProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (EngiesChaosModVariables.MapVariables.get(world).ChallengeToggle == true) {
			if (EngiesChaosModVariables.MapVariables.get(world).BYEBYE == false) {
				if (entity.getPersistentData().getDouble("plushtimeuntildeathforall") >= 20) {
			...
			} else if (EngiesChaosModVariables.MapVariables.get(world).BYEBYE == true) {
