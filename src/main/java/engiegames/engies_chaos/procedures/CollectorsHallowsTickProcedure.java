...
public class CollectorsHallowsTickProcedure {
	public static void execute(LevelAccessor world, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getOrCreateTag().getDouble("engiegameshallowscythestatclockcount") > 0) {
			itemstack.getOrCreateTag().putDouble("collectorshallowskillcount", (itemstack.getOrCreateTag().getDouble("engiegameshallowscythestatclockcount")));
			itemstack.getOrCreateTag().putDouble("engiegameshallowscythestatclockcount", 0);
		}
		if (itemstack.getOrCreateTag().getDouble("collectorshallowskillcount") < 10000) {
			itemstack.getOrCreateTag().putBoolean("givenupgradeitem", false);
		} else if (itemstack.getOrCreateTag().getDouble("collectorshallowskillcount") >= 10000) {
