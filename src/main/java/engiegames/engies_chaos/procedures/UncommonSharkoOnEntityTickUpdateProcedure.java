				...
				if (world.getLevelData().getGameRules().getBoolean(EngiesChaosModGameRules.ENGIE_POC) == true) {
					...
					if (entity.getPersistentData().getDouble("SharkoStateTimerForChance") >= 120) {
						...
						if (!entity.isInWater() && !entity.isInLava()) {
							...
							if ((entity instanceof UncommonSharkoEntity _datEntI ? _datEntI.getEntityData().get(UncommonSharkoEntity.DATA_SharkoState) : 0) == 2) {
								if (Math.random() <= 0.5) {
									if (Math.random() <= 0.49) {
										if (entity instanceof UncommonSharkoEntity _datEntSetI)
											_datEntSetI.getEntityData().set(UncommonSharkoEntity.DATA_SharkoState, 3);
							...
							if ((entity instanceof UncommonSharkoEntity _datEntI ? _datEntI.getEntityData().get(UncommonSharkoEntity.DATA_SharkoState) : 0) == 3) {
			...
			if (EngiesChaosModVariables.MapVariables.get(world).difficultytoggle == true) {
				if (EngiesChaosModVariables.MapVariables.get(world).MobDifficulty > 0) {
					if (entity instanceof LivingEntity _livingEntity62 && _livingEntity62.getAttributes().hasAttribute(Attributes.ATTACK_DAMAGE))
						_livingEntity62.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue((10 * 2.5 * EngiesChaosModVariables.MapVariables.get(world).MobDifficulty));
				}
			}
		}
	}
