...
public class RegularHallowsTickProcedure {
	public static void execute(LevelAccessor world, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getOrCreateTag().getDouble("engiegameshallowscythestatclockcount") > 0) {
			itemstack.getOrCreateTag().putDouble("regularhallowskillcount", (itemstack.getOrCreateTag().getDouble("engiegameshallowscythestatclockcount")));
			itemstack.getOrCreateTag().putDouble("engiegameshallowscythestatclockcount", 0);
		}
		if (itemstack.getOrCreateTag().getDouble("regularhallowskillcount") < 10000) {
			itemstack.getOrCreateTag().putBoolean("givenupgradeitem", false);
		} else if (itemstack.getOrCreateTag().getDouble("regularhallowskillcount") >= 10000) {
