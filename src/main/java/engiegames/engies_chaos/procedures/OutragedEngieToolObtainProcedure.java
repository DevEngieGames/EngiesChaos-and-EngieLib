	...
	public static void execute(LevelAccessor world, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.is(ItemTags.create(new ResourceLocation("engies_chaos:tools/nonantimatter")))) {
			...
			if ((entity instanceof ServerPlayer _plr6 && _plr6.level instanceof ServerLevel
					&& _plr6.getAdvancements().getOrStartProgress(_plr6.server.getAdvancements().getAdvancement(new ResourceLocation("engies_chaos:antimatter_outraged_tools_obtained"))).isDone()) == false) {
				if (entity instanceof ServerPlayer _player) {
