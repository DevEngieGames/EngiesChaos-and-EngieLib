		...
		if (!world.isClientSide()) {
			if (EngiesChaosModVariables.MapVariables.get(world).doomsdaychance < 6.25) {
				...
				for (Entity entityiterator : new ArrayList<>(world.players())) {
					if (entityiterator.getData(EngiesChaosModVariables.PLAYER_VARIABLES).DoomsdayRiskTrackToggle == true) {
				...
				if (Mth.nextDouble(RandomSource.create(), 0, 100) + Math.random() <= EngiesChaosModVariables.MapVariables.get(world).doomsdaychance) {
					...
					for (Entity entityiterator : new ArrayList<>(world.players())) {
						if (EngiesChaosModVariables.MapVariables.get(world).DoomsDayStart == true) {
							if (entityiterator.getData(EngiesChaosModVariables.PLAYER_VARIABLES).DoomsdayTrackToggle == true) {
								if (entityiterator instanceof Player _player && !_player.level().isClientSide())
							...
							}
							{
								...
								if (!_ent.level().isClientSide() && _ent.getServer() != null) {
									_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null,
											4, _ent.getName().getString(), _ent.getDisplayName(), _ent.level().getServer(), _ent), "EngieLib EChaos summondoomsday");
								}
							}
						}
					});
