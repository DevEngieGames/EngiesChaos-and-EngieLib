			...
			if (world.getLevelData().getGameRules().getBoolean(EngiesChaosModGameRules.ENGIE_POC) == true) {
				...
				if (entity.getPersistentData().getDouble("SharkoStateTimerForChance") >= 60) {
					...
					if (!entity.isInWater() && !entity.isInLava()) {
						...
						if ((entity instanceof SharkoEntity _datEntI ? _datEntI.getEntityData().get(SharkoEntity.DATA_SharkoState) : 0) == 2) {
							if (Math.random() <= 0.5) {
								if (Math.random() <= 0.49) {
									if (entity instanceof SharkoEntity _datEntSetI)
										_datEntSetI.getEntityData().set(SharkoEntity.DATA_SharkoState, 3);
						...
						if ((entity instanceof SharkoEntity _datEntI ? _datEntI.getEntityData().get(SharkoEntity.DATA_SharkoState) : 0) == 3) {
							if (Math.random() >= 0.75) {
								if (entity instanceof SharkoEntity _datEntSetI)
									_datEntSetI.getEntityData().set(SharkoEntity.DATA_SharkoState, 2);
								if (Math.random() <= 0.49) {
			...
			if (EngiesChaosModVariables.MapVariables.get(world).difficultytoggle == true) {
				if (EngiesChaosModVariables.MapVariables.get(world).MobDifficulty > 0) {
					if (entity instanceof LivingEntity _livingEntity59 && _livingEntity59.getAttributes().hasAttribute(Attributes.ATTACK_DAMAGE))
						_livingEntity59.getAttribute(Attributes.ATTACK_DAMAGE).setBaseValue((5 * 2.5 * EngiesChaosModVariables.MapVariables.get(world).MobDifficulty));
				}
			}
		}
	}
