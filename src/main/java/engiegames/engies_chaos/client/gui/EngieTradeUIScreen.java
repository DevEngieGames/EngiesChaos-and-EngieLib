...
public class EngieTradeUIScreen extends AbstractContainerScreen<EngieTradeUIMenu> implements EngiesChaosModScreens.ScreenAccessor {
	...
	private static final ResourceLocation texture = ResourceLocation.parse("engies_chaos:textures/screens/engie_trade_ui.png");
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (mouseX > leftPos + 69 && mouseX < leftPos + 93 && mouseY > topPos + 4 && mouseY < topPos + 28) {
			guiGraphics.renderTooltip(font, Component.translatable("gui.engies_chaos.engie_trade_ui.tooltip_click_this_to_trade_for_a_scythe"), mouseX, mouseY);
			customTooltipShown = true;
		}
		if (mouseX > leftPos + 96 && mouseX < leftPos + 120 && mouseY > topPos + 4 && mouseY < topPos + 28) {
			guiGraphics.renderTooltip(font, Component.translatable("gui.engies_chaos.engie_trade_ui.tooltip_click_this_to_trade_for_a_ban_ha"), mouseX, mouseY);
			customTooltipShown = true;
		}
	...
	public void init() {
		super.init();
		button_swap = Button.builder(Component.translatable("gui.engies_chaos.engie_trade_ui.button_swap"), e -> {
		...
		scythetrade = Checkbox.builder(Component.translatable("gui.engies_chaos.engie_trade_ui.scythetrade"), this.font).pos(this.leftPos + 71, this.topPos + 6).onValueChange((checkbox, value) -> {
			if (!menuStateUpdateActive)
				menu.sendMenuStateUpdate(entity, 1, "scythetrade", value, false);
		}).build();
		this.addRenderableWidget(scythetrade);
		bantrade = Checkbox.builder(Component.translatable("gui.engies_chaos.engie_trade_ui.bantrade"), this.font).pos(this.leftPos + 98, this.topPos + 6).onValueChange((checkbox, value) -> {
			if (!menuStateUpdateActive)
