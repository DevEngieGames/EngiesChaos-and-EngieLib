...
import engiegames.engies_chaos.network.LegendaryTradeUIButtonMessage;
...
public class LegendaryTradeUIScreen extends AbstractContainerScreen<LegendaryTradeUIMenu> implements EngiesChaosModScreens.ScreenAccessor {
	...
	private static final ResourceLocation texture = ResourceLocation.parse("engies_chaos:textures/screens/legendary_trade_ui.png");
	...
	public void init() {
		super.init();
		button_trade = Button.builder(Component.translatable("gui.engies_chaos.legendary_trade_ui.button_trade"), e -> {
			int x = LegendaryTradeUIScreen.this.x;
			int y = LegendaryTradeUIScreen.this.y;
			if (true) {
				PacketDistributor.sendToServer(new LegendaryTradeUIButtonMessage(0, x, y, z));
