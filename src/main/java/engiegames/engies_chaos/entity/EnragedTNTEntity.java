...
import engiegames.engies_chaos.procedures.TNTSpawnTNTDeathProcedure;
...
public class EnragedTNTEntity extends Monster {
	public EnragedTNTEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EngiesChaosModEntities.ENRAGED_TNT.get(), world);
	}
	...
	protected void registerGoals() {
		super.registerGoals();
		this.goalSelector.addGoal(1, new MeleeAttackGoal(this, 0.975, false) {
			@Override
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(EngiesChaosModItems.ENRAGED_COIN.get()));
	}
	...
	public void die(DamageSource source) {
		super.die(source);
		TNTSpawnTNTDeathProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
	}

	public static void init() {
		SpawnPlacements.register(EngiesChaosModEntities.ENRAGED_TNT.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
