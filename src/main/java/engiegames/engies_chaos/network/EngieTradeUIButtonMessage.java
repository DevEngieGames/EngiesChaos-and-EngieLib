...
import engiegames.engies_chaos.procedures.EngieSwapToAntimatterProcedure;
import engiegames.engies_chaos.EngiesChaosMod;

@EventBusSubscriber(bus = EventBusSubscriber.Bus.MOD)
public record EngieTradeUIButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {

	public static final Type<EngieTradeUIButtonMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(EngiesChaosMod.MODID, "engie_trade_ui_buttons"));
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		if (buttonID == 0) {
			...
			EngieSwapToAntimatterProcedure.execute(world, x, y, z, entity);
		}
	}

	@SubscribeEvent
	public static void registerMessage(FMLCommonSetupEvent event) {
		EngiesChaosMod.addNetworkMessage(EngieTradeUIButtonMessage.TYPE, EngieTradeUIButtonMessage.STREAM_CODEC, EngieTradeUIButtonMessage::handleData);
	}
}