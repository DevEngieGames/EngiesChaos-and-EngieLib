...
public class ScorchedRenderer extends MobRenderer<ScorchedEntity, Modelscorched<ScorchedEntity>> {
	public ScorchedRenderer(EntityRendererProvider.Context context) {
		super(context, new AnimatedModel(context.bakeLayer(Modelscorched.LAYER_LOCATION)), 0.5f);
	}
	...
	public ResourceLocation getTextureLocation(ScorchedEntity entity) {
		return new ResourceLocation("allaboutengie:textures/entities/scorched.png");
	}

	private static final class AnimatedModel extends Modelscorched<ScorchedEntity> {
		private final ModelPart root;
		private final HierarchicalModel animator = new HierarchicalModel<ScorchedEntity>() {
			...
			@Override
			public void setupAnim(ScorchedEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
