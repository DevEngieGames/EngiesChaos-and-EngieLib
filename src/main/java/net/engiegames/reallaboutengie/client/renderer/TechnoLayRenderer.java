...
public class TechnoLayRenderer extends MobRenderer<TechnoLayEntity, Modellouisandotherlay<TechnoLayEntity>> {
	public TechnoLayRenderer(EntityRendererProvider.Context context) {
		super(context, new AnimatedModel(context.bakeLayer(Modellouisandotherlay.LAYER_LOCATION)), 0.5f);
	}

	@Override
	protected void scale(TechnoLayEntity entity, PoseStack poseStack, float f) {
		poseStack.scale(1.75f, 1.75f, 1.75f);
	}

	@Override
	public ResourceLocation getTextureLocation(TechnoLayEntity entity) {
		return new ResourceLocation("allaboutengie:textures/entities/techno.png");
	}

	private static final class AnimatedModel extends Modellouisandotherlay<TechnoLayEntity> {
		private final ModelPart root;
		private final HierarchicalModel animator = new HierarchicalModel<TechnoLayEntity>() {
			...
			@Override
			public void setupAnim(TechnoLayEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
