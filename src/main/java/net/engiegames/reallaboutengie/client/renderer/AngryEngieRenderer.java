...
public class AngryEngieRenderer extends MobRenderer<AngryEngieEntity, Modelhostile<AngryEngieEntity>> {
	...
	public ResourceLocation getTextureLocation(AngryEngieEntity entity) {
		return new ResourceLocation("allaboutengie:textures/entities/angrynew_style1.png");
	}

	private static final class AnimatedModel extends Modelhostile<AngryEngieEntity> {
		private final ModelPart root;
		private final HierarchicalModel animator = new HierarchicalModel<AngryEngieEntity>() {
			...
			@Override
			public void setupAnim(AngryEngieEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
