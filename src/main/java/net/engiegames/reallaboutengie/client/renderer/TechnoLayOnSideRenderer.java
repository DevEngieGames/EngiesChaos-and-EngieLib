...
public class TechnoLayOnSideRenderer extends MobRenderer<TechnoLayOnSideEntity, Modellouisandotherlayonside<TechnoLayOnSideEntity>> {
	public TechnoLayOnSideRenderer(EntityRendererProvider.Context context) {
		super(context, new AnimatedModel(context.bakeLayer(Modellouisandotherlayonside.LAYER_LOCATION)), 0.5f);
	}

	@Override
	protected void scale(TechnoLayOnSideEntity entity, PoseStack poseStack, float f) {
		poseStack.scale(1.75f, 1.75f, 1.75f);
	}

	@Override
	public ResourceLocation getTextureLocation(TechnoLayOnSideEntity entity) {
		return new ResourceLocation("allaboutengie:textures/entities/techno.png");
	}

	private static final class AnimatedModel extends Modellouisandotherlayonside<TechnoLayOnSideEntity> {
		private final ModelPart root;
		private final HierarchicalModel animator = new HierarchicalModel<TechnoLayOnSideEntity>() {
			...
			@Override
			public void setupAnim(TechnoLayOnSideEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
