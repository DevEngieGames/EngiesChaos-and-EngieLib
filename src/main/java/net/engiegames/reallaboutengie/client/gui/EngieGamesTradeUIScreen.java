...
import net.engiegames.reallaboutengie.network.EngieGamesTradeUIButtonMessage;
...
public class EngieGamesTradeUIScreen extends AbstractContainerScreen<EngieGamesTradeUIMenu> implements AllaboutengieModScreens.ScreenAccessor {
	...
	protected void renderLabels(PoseStack ms, int mouseX, int mouseY) {
		this.font.draw(ms, Component.translatable("gui.allaboutengie.engie_games_trade_ui.label_trade_ui_state_normal"), 5, -11, -1);
	}

	@Override
	public void init() {
		super.init();
		button_trade = new Button(this.leftPos + 8, this.topPos + 29, 51, 20, Component.translatable("gui.allaboutengie.engie_games_trade_ui.button_trade"), e -> {
		...
		button_swap = new Button(this.leftPos + 8, this.topPos + 6, 51, 20, Component.translatable("gui.allaboutengie.engie_games_trade_ui.button_swap"), e -> {
			int x = EngieGamesTradeUIScreen.this.x;
			int y = EngieGamesTradeUIScreen.this.y;
			if (true) {
				AllaboutengieMod.PACKET_HANDLER.sendToServer(new EngieGamesTradeUIButtonMessage(1, x, y, z));
