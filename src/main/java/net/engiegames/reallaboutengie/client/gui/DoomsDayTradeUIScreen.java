...
import net.engiegames.reallaboutengie.network.DoomsDayTradeUIButtonMessage;
...
public class DoomsDayTradeUIScreen extends AbstractContainerScreen<DoomsDayTradeUIMenu> implements AllaboutengieModScreens.ScreenAccessor {
	...
	protected void renderLabels(PoseStack ms, int mouseX, int mouseY) {
		this.font.draw(ms, Component.translatable("gui.allaboutengie.dooms_day_trade_ui.label_trade_ui_state_normal"), 5, -11, -1);
	}

	@Override
	public void init() {
		super.init();
		button_trade = new Button(this.leftPos + 9, this.topPos + 29, 51, 20, Component.translatable("gui.allaboutengie.dooms_day_trade_ui.button_trade"), e -> {
		...
		button_swap = new Button(this.leftPos + 9, this.topPos + 6, 51, 20, Component.translatable("gui.allaboutengie.dooms_day_trade_ui.button_swap"), e -> {
			int x = DoomsDayTradeUIScreen.this.x;
			int y = DoomsDayTradeUIScreen.this.y;
			if (true) {
				AllaboutengieMod.PACKET_HANDLER.sendToServer(new DoomsDayTradeUIButtonMessage(1, x, y, z));
				DoomsDayTradeUIButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		});
		this.addRenderableWidget(button_swap);
		scythetrade = new Checkbox(this.leftPos + 71, this.topPos + 6, 20, 20, Component.translatable("gui.allaboutengie.dooms_day_trade_ui.scythetrade"), false) {
			@Override
			public void onPress() {
				super.onPress();
