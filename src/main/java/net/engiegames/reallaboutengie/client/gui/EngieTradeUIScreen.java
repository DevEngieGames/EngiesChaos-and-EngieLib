...
import net.engiegames.reallaboutengie.network.EngieTradeUIButtonMessage;
...
public class EngieTradeUIScreen extends AbstractContainerScreen<EngieTradeUIMenu> implements AllaboutengieModScreens.ScreenAccessor {
	...
	protected void renderLabels(PoseStack ms, int mouseX, int mouseY) {
		this.font.draw(ms, Component.translatable("gui.allaboutengie.engie_trade_ui.label_trade_ui_state_normal"), 5, -11, -1);
	}

	@Override
	public void init() {
		super.init();
		button_trade = new Button(this.leftPos + 9, this.topPos + 29, 51, 20, Component.translatable("gui.allaboutengie.engie_trade_ui.button_trade"), e -> {
		...
		button_swap = new Button(this.leftPos + 9, this.topPos + 6, 51, 20, Component.translatable("gui.allaboutengie.engie_trade_ui.button_swap"), e -> {
			int x = EngieTradeUIScreen.this.x;
			int y = EngieTradeUIScreen.this.y;
			if (true) {
				AllaboutengieMod.PACKET_HANDLER.sendToServer(new EngieTradeUIButtonMessage(1, x, y, z));
