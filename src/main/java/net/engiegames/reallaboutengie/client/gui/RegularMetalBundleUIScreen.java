...
import net.engiegames.reallaboutengie.procedures.RegularBundleDisplayNameCheckProcedure;
import net.engiegames.reallaboutengie.network.RegularMetalBundleUIButtonMessage;
...
public class RegularMetalBundleUIScreen extends AbstractContainerScreen<RegularMetalBundleUIMenu> implements AllaboutengieModScreens.ScreenAccessor {
	...
	private static final ResourceLocation texture = new ResourceLocation("allaboutengie:textures/screens/regular_metal_bundle_ui.png");
	...
	public void init() {
		...
		ChangeBundleName.setSuggestion(Component.translatable("gui.allaboutengie.regular_metal_bundle_ui.ChangeBundleName").getString());
		this.addWidget(this.ChangeBundleName);
		button_set_name = new Button(this.leftPos + 130, this.topPos + 4, 66, 20, Component.translatable("gui.allaboutengie.regular_metal_bundle_ui.button_set_name"), e -> {
			int x = RegularMetalBundleUIScreen.this.x;
			int y = RegularMetalBundleUIScreen.this.y;
			if (true) {
				AllaboutengieMod.PACKET_HANDLER.sendToServer(new RegularMetalBundleUIButtonMessage(0, x, y, z));
