					...
					if (AllaboutengieModVariables.MapVariables.get(world).ddaydialoguetimeblock == false) {
						...
						if (AllaboutengieModVariables.MapVariables.get(world).ddaydialoguetimeblock == false) {
							if (entity.getPersistentData().getDouble("DialogueCooldownStart") >= 36.1) {
								...
								if (world instanceof Level _level) {
									if (!_level.isClientSide()) {
										_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("allaboutengie:churchbells")), SoundSource.AMBIENT, (float) 0.5, 1);
									} else {
									...
									AllaboutengieMod.queueServerWork(188, () -> {
										...
										AllaboutengieMod.queueServerWork(200, () -> {
											...
											AllaboutengieMod.queueServerWork(200, () -> {
												...
												if (world instanceof Level _level) {
													if (!_level.isClientSide()) {
														_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("allaboutengie:doomsday_start")), SoundSource.MUSIC, (float) 0.5, 1);
													} else {
