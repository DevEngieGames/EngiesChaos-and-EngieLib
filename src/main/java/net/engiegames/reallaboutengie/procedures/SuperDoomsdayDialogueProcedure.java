					...
					if (AllaboutengieModVariables.MapVariables.get(world).sddaydialoguetimeblock == false) {
						...
						if (AllaboutengieModVariables.MapVariables.get(world).sddaydialoguetimeblock == false) {
							if (entity.getPersistentData().getDouble("SDDAYDialogueCooldownStart") >= 36.1) {
								...
								if (world instanceof Level _level) {
									if (!_level.isClientSide()) {
										_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("allaboutengie:churchbells2")), SoundSource.AMBIENT, (float) 0.5, 1);
									} else {
												...
												AllaboutengieMod.queueServerWork(200, () -> {
													...
													AllaboutengieMod.queueServerWork(200, () -> {
														...
														AllaboutengieMod.queueServerWork(200, () -> {
															...
															if (world instanceof Level _level) {
																if (!_level.isClientSide()) {
																	_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("allaboutengie:doomsday_start")), SoundSource.MUSIC, 1, 1);
																} else {
								...
								if (!(entity instanceof ServerPlayer _plr33 && _plr33.level instanceof ServerLevel
										&& _plr33.getAdvancements().getOrStartProgress(_plr33.server.getAdvancements().getAdvancement(new ResourceLocation("allaboutengie:rise"))).isDone())) {
