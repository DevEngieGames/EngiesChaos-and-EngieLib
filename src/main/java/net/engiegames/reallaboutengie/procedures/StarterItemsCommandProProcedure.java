...
public class StarterItemsCommandProProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		if (entity == null)
			return;
		if (world.getLevelData().getGameRules().getBoolean(AllaboutengieModGameRules.ENRAGED_ZOMBIES_TOGGLE) == true) {
			{
				Entity _ent = entity;
		...
		} else if (world.getLevelData().getGameRules().getBoolean(AllaboutengieModGameRules.ENRAGED_ZOMBIES_TOGGLE) == false) {
			{
				Entity _ent = entity;
				if (!_ent.level.isClientSide() && _ent.getServer() != null) {
					_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level instanceof ServerLevel ? (ServerLevel) _ent.level : null, 4,
