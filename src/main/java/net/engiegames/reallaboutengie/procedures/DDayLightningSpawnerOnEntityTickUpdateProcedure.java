		...
		if ((world.getBlockState(new BlockPos(x, y - 1, z))).getBlock() == Blocks.AIR) {
			...
			if (AllaboutengieModVariables.MapVariables.get(world).ddaystart == true || AllaboutengieModVariables.MapVariables.get(world).sddaystart == true || AllaboutengieModVariables.MapVariables.get(world).thestart == true) {
				if (world.getLevelData().getGameRules().getBoolean(AllaboutengieModGameRules.EXTREME_DOOMSDAY_LIGHTNING) == false) {
					...
					&& !(AllaboutengieModVariables.MapVariables.get(world).ddaystart == true || AllaboutengieModVariables.MapVariables.get(world).sddaystart == true || AllaboutengieModVariables.MapVariables.get(world).thestart == true)) {
				if (world.getLevelData().getGameRules().getBoolean(AllaboutengieModGameRules.HEAVY_LIGHTNING) == true) {
					if (Math.random() < 0.5) {
						if (world instanceof Level _level) {
							if (!_level.isClientSide()) {
								_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("allaboutengie:customlightning")), SoundSource.AMBIENT, (float) 0.25, 1);
							} else {
						...
						if (world instanceof Level _level) {
							if (!_level.isClientSide()) {
								_level.playSound(null, new BlockPos(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("allaboutengie:customlightning2")), SoundSource.AMBIENT, (float) 0.25, 1);
							} else {
