...
public class DeepslateEngieOreBlockDestroyedProcedure {
					...
					if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getEnchantmentLevel(AllaboutengieModEnchantments.ENGIES_BLESSING.get()) > 15) {
						...
						for (int index0 = 0; index0 < 15; index0++) {
							if (Math.random() >= 0.5) {
								...
								if (world instanceof ServerLevel _level) {
									ItemEntity entityToSpawn = new ItemEntity(_level, x, y, z, new ItemStack(AllaboutengieModBlocks.DEEPSLATE_ANTIMATTER_ENGIE_ORE.get()));
									entityToSpawn.setPickUpDelay(10);
									_level.addFreshEntity(entityToSpawn);
								}
							}
