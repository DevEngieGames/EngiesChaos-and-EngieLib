	...
	public static void execute(LevelAccessor world, Entity entity, ItemStack itemstack) {
		...
		if (itemstack.getOrCreateTag().getDouble("engiegameshallowscythestatclockcount") < 10) {
			...
			if (entity instanceof Player _player) {
				ItemStack _stktoremove = new ItemStack(AllaboutengieModItems.ENGIE_GAMES_HALLOW_SCYTHE.get());
				_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 1, _player.inventoryMenu.getCraftSlots());
			}
			if (entity instanceof Player _player) {
				ItemStack _setstack = new ItemStack(AllaboutengieModItems.ENGIE_GAMES_GOLDEN_HALLOW_SCYTHE.get()).copy();
				_setstack.setCount(1);
				ItemHandlerHelper.giveItemToPlayer(_player, _setstack);
			}
			AllaboutengieMod.queueServerWork(1, () -> {
				{
					CompoundTag _nbtTag = new ItemStack(AllaboutengieModItems.ENGIE_GAMES_HALLOW_SCYTHE.get()).getTag();
					if (_nbtTag != null)
						new ItemStack(AllaboutengieModItems.ENGIE_GAMES_GOLDEN_HALLOW_SCYTHE.get()).setTag(_nbtTag.copy());
				}
			});
			AllaboutengieMod.queueServerWork(2, () -> {
				new ItemStack(AllaboutengieModItems.ENGIE_GAMES_GOLDEN_HALLOW_SCYTHE.get()).getOrCreateTag().putDouble("engiegameshallowscythestatclockcount", 20210);
			});
		}
	}
}